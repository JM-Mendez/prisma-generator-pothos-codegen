...
export async function generateCrud(
  ...
  const exposedModels = new Set(config.crud.exposedModels ?? []);

  const modelNames = dmmf.datamodel.models
    .map((model) => model.name)
    .filter((modelName) => {
      return !exposedModels.length || exposedModels.has(modelName);
    });

  // Generate CRUD directories (e.g. User, Comment, ...)
  await Promise.all(
    modelNames.map((model) => generateModel(config, dmmf, model)),
  );
